...
def _resolve_asset_root(value: str | Path | None) -> Path:
    ...
    for candidate in candidates:
        ...
        if all((resolved / item).is_dir() for item in REQUIRED_MODEL_DIRS):
            return resolved
    raise RuntimeError(
        "SOLVE_LITE_ASSETS_UNAVAILABLE: provide --asset-root or SOLVE_LITE_ASSET_ROOT; "
...
def run_frozen_case(
    ...
    *,
    ...
    runtime = FrozenKernel(asset_root)
    ...
    attach_reward(
        ...
        quality_gate_reason="FROZEN_KERNEL_PASS",
        local_decisions=len(result["answers"]),
        locale=locale,
    ...
    result["user_visible_settlement"] = render_settlement(result, locale)
